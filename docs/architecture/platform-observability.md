...
`GET /api/admin/organizations/{organizationId}/inbox` returns 30 threads per page. Its `/messages` child returns 50 messages with a cursor containing timestamp and ID; the ID breaks timestamp ties. Both SQL and APIs filter organization before channel/search/pagination. Older pages remain available without a lifetime transcript cap.
