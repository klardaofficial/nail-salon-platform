...
Customer list/cancel operations always filter by the current contact. Owner mutations load a salon and compare its `business_id` with the caller's stored membership. Technician booking queries use only the matching technician IDs; time off checks the submitted technician ID belongs to the actor.
