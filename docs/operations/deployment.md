...
5. Create approved technician notification templates in the deployed bot language:
   - Confirmed template body parameters: salon name, local appointment label, booking ID.
   - Cancelled template body parameter: booking ID.
6. Record the phone number ID, business account ID, access token, app secret, verify token, and approved template names.
