...
A booking requires an active singleton business, a future timestamp, and customer agreement, and auto-confirms. Salon selection is optional whether or not active salons exist, and the platform timezone always applies. The assistant offers active salons as a reference but never assigns one without the customer's selection. A supplied stale/inactive salon is rejected. Services, duration, capacity, pricing, technician, and attendance remain optional.

`bookings.technician_ref` remains a nullable UUID without a foreign key. Unresolved references may persist; a valid matching technician supplies a name snapshot and notification recipient. References to another salon are not assigned. Whenever the selected salon has active technicians, a customer may choose one or continue without preference. Missing display values use [N/A], never a fabricated catalog record.

Collect only missing required details, one focused question at a time. When salons exist, offer an optional salon reference before confirmation; the customer can skip it. Offer an optional technician preference only after a salon is selected and it has active technicians. A clear instruction to book the exact supplied details counts as agreement; otherwise summarize and obtain agreement once. Do not delay after an optional preference is skipped, and omit absent salon or technician details from customer confirmations.
...
2026-09-16 service-scope revision: service configuration is advisory, not a booking constraint. A service with no `service_salons` restriction is offered for every selected salon; a restricted service can target one or more salons. The customer is asked about helpful service choices only after selecting a salon or No preference. The assistant accepts and snapshots free-text/custom services regardless of catalog membership or salon scope.
