...
This simulator does not emulate Meta customer-service windows, template approval, delivery receipts, media upload, or image generation. Bot text and all control labels follow the person's conversation language, using `BOT_LOCALE` only as an initial reference; OpenAI-backed functions require the configured OpenAI key and incur normal usage.
