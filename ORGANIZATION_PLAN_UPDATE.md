...
### Customer click-to-chat QR code

- `phone_number_id` is an internal Graph API identifier and cannot be used as the customer destination. A customer QR code requires the real WhatsApp phone number registered to that ID, normalized to international E.164 digits without `+`, spaces, or punctuation.
- During provider validation, query Meta for the configured phone-number ID, verify it belongs to the configured WABA, and save the returned display phone number plus its normalized E.164 digits on the organization's provider settings. Changing the phone-number ID clears the saved/validated number and disables the QR until validation succeeds again. Do not trust an administrator-entered display number as proof of the provider mapping.
- Generate the QR locally from `https://wa.me/{e164Digits}`. Do not add a prefilled message and do not call an external QR service. Scanning opens the organization's WhatsApp conversation; the customer writes and sends the first message, which creates the inbound event and triggers the AI reply.
- Show the real phone number, clickable URL, copy action, and downloadable/printable QR on the authorized organization settings page only when the organization is active and the phone mapping has current successful validation. The phone number and click-to-chat link are customer-facing values, not credentials.
- The QR contains no organization ID, access token, verify token, bypass value, or authorization claim. Organization selection still comes exclusively from the signed webhook's receiving phone-number ID/WABA mapping. Scanning the QR does not bypass WhatsApp consent, the customer's send action, signature verification, or normal conversation processing.

### Credential storage and dashboard access

- Store root Meta credentials, organization provider credentials, and organization OpenAI keys as plain text in server-only database tables. This project explicitly accepts database-level plain-text storage to avoid a separate application encryption key and its environment/rotation workflow. Root settings and their platform audit records are platform-scoped; tenant settings and their audit records remain organization-scoped. The browser never accesses credential tables directly, and RLS plus route authorization restrict all reads and writes.
- Authorized settings APIs may return saved **Meta** values for editing: only system administrators may read/write root values; system administrators and assigned organization administrators may read/write an organization's saved overrides. Use password inputs with hide/show controls for access tokens, app secrets, and verify tokens. Hiding an input is presentation; route-handler authorization enforces access.
