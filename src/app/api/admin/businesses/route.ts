...
export async function POST(request: Request) {
  try {
    const guard = await requireApiAdmin();
    if (guard.error) return guard.error;
    return apiSuccess(await createResource("businesses", await request.json()), { status: 201 });
  } catch (error) {
    return apiException(error);
  }
}
...
export async function DELETE(request: Request) {
  try {
    const guard = await requireApiAdmin();
    if (guard.error) return guard.error;
    return apiSuccess(await deactivateResource("businesses", await request.json()));
  } catch (error) {
    return apiException(error);
  }
}
